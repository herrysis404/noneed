...
from tok.helper_funcs.download_aria_p_n import aria_start
from tok.helper_funcs.youtube_dl_button import youtube_dl_call_back
from tok.plugins.status_message_fn import cancel_message_f
from tok import (
    MAX_MESSAGE_LENGTH,
    AUTH_CHANNEL
)
    ...
    if "|" in cb_data:
        ...
        if cb_data.startswith("cancel"):
            ...
            if update.from_user.id in AUTH_CHANNEL:
                g_d_list = ['app.json', 'venv', 'rclone.conf', '.gitignore', '_config.yml', 'COPYING', 'Dockerfile', 'DOWNLOADS', 'Procfile', '.heroku', '.profile.d', 'rclone.jpg', 'README.md', 'requirements.txt', 'runtime.txt', 'start.sh', 'tok', 'gautam', 'Torrentleech-Gdrive.txt', 'vendor']
